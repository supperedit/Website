...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  const entries: HerbariumEntry[] = await herbariumRes.json();
  ...
  const image = entry.image ? `${SITE_URL}/img/herbarium/${entry.slug}` : `${SITE_URL}/og-image.jpg`;
  const url = `${SITE_URL}/herbarium/${entry.slug}`;
