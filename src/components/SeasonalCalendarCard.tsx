...
function pickSeasonalRecipes(
  ...
  const seasonalSlugs = new Set(
    (monthData?.items ?? []).map((i) => i.slug),
  );

  const seasonal = recipes.filter((r) =>
    [...seasonalSlugs].some(
      (slug) =>
        r.slug?.includes(slug) ||
        r.title?.toLowerCase().includes(slug),
    ),
  );
...
function buildGrid(
  ...
  const firstWeekday =
    (new Date(year, monthIndex0, 1).getDay() + 6) % 7;

  const today = new Date();

  const monthIdeas = dinnerIdeas.filter(
    (e) => e.month === monthIndex0 + 1,
  );
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    cells.push({
      ...
      isToday:
        ...
        today.getDate() === day,
      idea:
        monthIdeas.find((e) => e.day === day) ?? null,
...
export default function SeasonalCalendarCard() {
  ...
  const [monthIndex0, setMonthIndex0] = useState(
    today.getMonth(),
  );
  ...
  const ideaDays = useMemo(() => {
    ...
    dinnerIdeas
      .filter((e) => e.month === monthIndex0 + 1)
      .forEach((e) => s.add(e.day));
  ...
  return (
    <div className="sc-root">
      <header className="sc-heading">
        <h2>Ein guter Anlass.</h2>
      </header>

      <div className="sc-layout">
        <div className="sc-calendar-column">
          <h3 className="sc-subheading">
            Ideen für deinen Monat
          </h3>
            ...
            <div className="sc-weekdays">
              {WEEKDAY_FULL.map((label, i) => (
                <div key={label} className="sc-dow">
                  <span className="sc-dow-full">
                    {label}
                  </span>
                  <span className="sc-dow-short">
                    {WEEKDAY_SHORT[i]}
                  </span>
                </div>
              ))}
                ...
                week.map((cell, di) => {
                  ...
                  if (cell.day === null) {
                    return (
                      <div
                        key={key}
                        className="sc-cell sc-cell-empty"
                      />
                    );
                  }
                  ...
                  return (
                    <div
                      key={key}
                      className={`sc-cell${
                        cell.isToday ? " sc-cell-today" : ""
                      }${
                        hasRecipe ? " sc-cell-recipe" : ""
                      }${
                        hasIdea ? " sc-cell-idea" : ""
                      }`}
                    >
                      {!hasRecipe && !hasIdea && (
                        <span className="sc-day-num">
                          {cell.day}
                        </span>
                      )}
              ...
              >
                ...
                >
                  ...
                  <span className="sc-popup-eyebrow">
                    {activePopup.idea.day}.{" "}
                    {MONTH_NAMES[activePopup.idea.month - 1]}
                  </span>

                  <h3
                    id="sc-popup-title"
                    className="sc-popup-title"
                  >
                    {activePopup.idea.title}
                  </h3>

                  <p className="sc-popup-idea">
                    {activePopup.idea.idea}
                  </p>

                  {(activePopup.idea.onTheTable ||
                    activePopup.idea.kochen) && (
                    <div className="sc-popup-section">
                      <p className="sc-popup-label">
                        Auf dem Tisch
                      </p>
                  ...
                  {activePopup.idea.afterDinner && (
                    <div className="sc-popup-section">
                      ...
                      <ul className="sc-popup-list">
                        {activePopup.idea.afterDinner.map(
                          (e) => <li key={e}>{e}</li>,
                        )}
                      </ul>
                    </div>
        ...
        <aside
          className="sc-dinner"
          aria-labelledby="sc-dinner-title"
        >
          <p className="sc-subheading">
            Eine Idee für {currentMonthName}
          </p>

          {featuredIdea ? (
            <>
              <h3 id="sc-dinner-title">
                {featuredIdea.title}
              </h3>

              <p className="sc-dinner-intro">
                {featuredIdea.idea}
              </p>

              {(
                featuredIdea.onTheTable ||
                featuredIdea.kochen
              )?.length ? (
                <div className="sc-dinner-detail">
                  <h4>Auf dem Tisch</h4>
                  <ul>
                    {(
                      featuredIdea.onTheTable ||
              ...
              {featuredIdea.drink && (
                <div className="sc-dinner-detail">
                  <h4>Im Glas</h4>
                  <p>{featuredIdea.drink}</p>
                </div>
              )}
            </>
          ) : (
            <h3 id="sc-dinner-title">
              Ein Abend nach deinem Geschmack.
            </h3>
          )}

          <Link
            to="/rezepte"
            className="sc-dinner-link"
          >
            Rezepte zum Teilen entdecken →
          </Link>
        </aside>
      </div>
      ...
      <style>{`
        ...
        .sc-heading h2 {
          font: 400 clamp(36px, 4.5vw, 58px)/1.15 var(--font-display);
          color: inherit;
          max-width: 780px;
        }
        ...
        .sc-cell {
          ...
          /* Small inset so absolutely-positioned children (inset: 0) don't
             sit flush against the cell border. */
          padding: 3px;
          box-sizing: border-box;
          overflow: visible;
        }

        .sc-cell:nth-child(7n) {
          border-right: none;
        }

        .sc-cell-empty {
          background: rgba(43, 18, 16, 0.015);
        }
        ...
        .sc-popup-eyebrow {
          ...
          color: var(--color-terracotta, #c4714a);
          margin-bottom: 3px;
        }
        ...
        .sc-dinner h3 {
          font: 400 clamp(28px, 3vw, 40px)/1.2 var(--font-display);
          color: inherit;
          margin: 0 0 20px;
        }
        ...
        .sc-dinner-detail {
          padding-top: 18px;
          margin-top: 20px;
          border-top: 1px solid rgba(247, 246, 236, .35);
        }
